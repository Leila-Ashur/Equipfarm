...
import "react-toastify/dist/ReactToastify.css";const Signup = () => {
  ...
  const [user, setUser] = useState({
    ...
    location: "",
    role: "",
  });  const [showSuccessToast, setShowSuccessToast] = useState(false);  const onSubmit = async (event: FormEvent) => {
    event.preventDefault();    try {
      const response = await axios.post(
        "https://678e-41-80-114-248.ngrok-free.app/api/register/",
        user
      );
      const data = response.data;
      console.log("Data posted successfully:", data);      setShowSuccessToast(true);      setTimeout(() => {
    ...
    <div className="fixed top-0 left-0 w-screen h-screen bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url(/images/green.webp)" }}>
      <div className="flex flex-col sm:flex-row font-poppins text-xl p-14">
        ...
        </div>        <div className="w-full bg-white flex flex-col p-16 sm:w-1/2 sm:p-20 mt-[-1.%]">
    ...
    <form className="flex flex-col sm:w-full h-auto justify-center " onSubmit={onSubmit}>
      <div className="mb-4">
        <label className="text-sm text-black font-black text-custom-color" htmlFor="email round-20">
          USER NAME
        </label>        <div className="relative text-black flex item-center border-solid border-2 border-black  justify-center">
      ...
      <div className="flex items-center mt-4">
        <div className="text-sm text-[#4B4B4B]">
          ...
          <button onClick={() => router.push("/dashbordlogin")} style={{ fontWeight: 'bold' }} className="ml-1">SignIn</button>
        </div>
      </div>
    </form>
  </div>
