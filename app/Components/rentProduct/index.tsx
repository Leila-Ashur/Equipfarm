...
//  console.log("products in cart ",)
  const { ProductData:GetProducts, loading, error } = useGetProducts();
  const selectedProduct =GetProducts.find((product) => product.id === Number(id));
console.log("products in hooks ",GetProducts)
  ...
  return (
    <div className="grid lg:grid-cols-10 gap-4 min-h-[90vh] md:gap-x-8 ">
      ...
      <div className="w-full flex flex-col items-center justify-end min-h-[500px]">
        <div className="">
          <img
            ...
            alt={selectedProduct?.equipment_name}
            className="w-96 h-96"
          />
        </div>
      </div>
      ...
      <h4 className="font-semibold text-[32px] md:text-[48px]">{selectedProduct?.equipment_name}</h4>
      <p className="md:text-[32px]">{selectedProduct?.is_available ?"available":" out of stock"}</p>
      ...
      onClick={() => selectedProduct && addToCart({
